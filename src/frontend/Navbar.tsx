...
export default function Navbar({ userStats }: NavbarProps) {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(errorData.error || "An error occurred while deleting your account.");
      }
    } catch (e) {
  ...
  const displayPoints = userStats?.totalPoints ?? (user as any)?.totalPoints ?? 0;
  const displaySolved = userStats?.solvedCount ?? (user as any)?.solvedCount ?? 0;
          ...
          {user && (
            <div className="flex items-center gap-4 sm:gap-6">
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-xs font-semibold text-indigo-400">
                  <span>🏆</span>
                  <span>{displayPoints} pts</span>
                </div>
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-xs font-semibold text-cyan-400">
                  <span>✅</span>
