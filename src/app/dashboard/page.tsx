...
export default function Dashboard() {
  ...
  const [pinnedUsers, setPinnedUsers] = useState<UserSession[]>([]);
  ...
  const [peerResults, setPeerResults] = useState<UserSession[]>([]);
    ...
    try {
      ...
      if (statsRes.ok) {
        ...
        if (savedUser) {
          ...
          userObj.totalPoints = sData.totalPoints;
          userObj.solvedCount = sData.solvedCount;
      ...
      if (pinnedRes.ok) {
        const pData = await pinnedRes.json();
        setPinnedUsers(pData);
      }

      
      const progressRes = await fetch(`/api/leaderboard/progress?userId=${userId}`);
      if (progressRes.ok) {
        const prData = await progressRes.json();
        setWeeklyProgress(prData);
      }
    } catch (e) {
  ...
  const handleSolveToggle = async (questionId: string) => {
    ...
    try {
      ...
      console.log("Solve toggle error:", e);
    }
  };
  ...
  const handleSaveNotes = async (notes: string | null) => {
    if (!currentUser || !activeNoteQuestion) return;
    try {
  ...
  const renderChart = () => {
    ...
    const colors = ["#6366f1", "#06b6d4", "#a855f7", "#22c55e"]; // Indigo, Cyan, Purple, Green

   
    let maxSolve = 1;
    weeklyProgress.forEach((p) => {
      activeUsers.forEach((user) => {
        const val = Number(p[user] || 0);
      ...
      <div className="relative">
        <svg className="w-full h-auto overflow-visible" viewBox={`0 0 ${width} ${height}`}>
          ...
          {Array.from({ length: 4 }).map((_, i) => {
            ...
            return (
              <g key={i} className="opacity-10">
                ...
                <text x={padding - 8} y={y + 4} fill="white" className="text-[10px] text-right" textAnchor="end">
                  {val}
                </text>
              </g>
            );
          ...
          {weeklyProgress.map((p, idx) => {
            const x = getX(idx);
            return (
              <text key={idx} x={x} y={height - 12} fill="rgba(255,255,255,0.4)" className="text-[9px] font-medium" textAnchor="middle">
                {p.date}
              </text>
            );
          })}
           ...
           <div className="lg:col-span-2 space-y-8">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
               ...
               <div className="p-5 rounded-2xl glass-panel border border-white/5 flex items-center justify-between">
                ...
                <div className="h-12 w-12 rounded-xl bg-indigo-500/10 text-indigo-400 flex items-center justify-center text-xl font-bold">
                  🏆
                </div>
              </div>
               ...
               <div className="p-5 rounded-2xl glass-panel border border-white/5 flex items-center justify-between">
                ...
                <div className="h-12 w-12 rounded-xl bg-cyan-500/10 text-cyan-400 flex items-center justify-center text-xl font-bold">
                  ⚡
                </div>
              </div>
             ...
             <div className="p-6 rounded-2xl glass-panel border border-white/5 relative overflow-hidden">
              ...
              {weeklyProgress.length > 0 ? (
                ...
                <div className="h-32 flex items-center justify-center text-sm text-zinc-500">
                  No solve statistics calculated yet. Start solving problems to map charts.
                </div>
              )}
            </div>
             ...
             <div className="p-6 rounded-2xl glass-panel border border-white/5 space-y-6">
                ...
                <div>
                 ...
                 <div className="relative">
                  <input
                    ...
                    className="w-full md:w-64 rounded-xl border border-zinc-800 bg-zinc-950 p-2.5 pl-9 text-xs text-white placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-transparent transition"
                  />
                  <svg className="absolute left-3 top-3 h-4.5 w-4.5 text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
               ...
               <div className="flex flex-wrap items-center justify-between gap-3 pt-1 border-t border-zinc-800/60">
                 ...
                 <div className="flex items-center gap-1.5 bg-zinc-950/60 p-1 rounded-lg border border-zinc-900">
                  {["ALL", "SOLVED", "UNSOLVED", "STARRED"].map((status) => (
                    <button
                      key={status}
           ...
           <div className="space-y-8">
             ...
             <div className="p-6 rounded-2xl glass-panel border border-white/5 space-y-4">
              <div>
                ...
                <p className="text-xs text-zinc-500 mt-0.5">Search for student usernames to pin them to your comparative chart</p>
              </div>
               ...
               <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl border border-zinc-850 bg-zinc-950 p-3 pl-10 text-xs text-white placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                />
                <svg className="absolute left-3.5 top-3.5 h-4.5 w-4.5 text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
              </div>
               ...
               {peerSearch.trim() !== "" && (
                <div className="border border-zinc-800 bg-zinc-950 rounded-xl overflow-hidden divide-y divide-zinc-900 max-h-40 overflow-y-auto">
                  {isSearchingPeers ? (
                    <div className="p-3 text-xs text-zinc-500 text-center">Searching database...</div>
               ...
               {pinnedUsers.length > 0 && (
                <div className="pt-2 space-y-2 border-t border-zinc-850">
