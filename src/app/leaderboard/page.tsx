  ...
  export default function LeaderboardPage() {
    ...
    useEffect(() => {
      ...
      const loadLeaderboard = useCallback(async (userId:string) => {
        ...
        try{
            ...
            const pinnnedResponse = await fetch(`/api/social/pinned?userId=${userId}`);
            if(pinnnedResponse.ok){
                const pinnedData:UserSession[] = await pinnnedResponse.json();
      ...
      useEffect(()=>{
        ...
        const handlePinToggle = async (targetUserId:string, isCurrentlyPinned:boolean)=>{
            ...
            try{
                ...
                if (pinnedRes.ok) {
                const pData: UserSession[] = await pinnedRes.json();
        ...
        const fileteredLeaderboard = leaderboard.filter(student=>{
            const matchesStudent = student.username.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    return(
      ...
      <main className="flex-1 mx-auto max-w-5xl w-full px-4 sm:px-6 lg:px-8 py-8">
         ...
         <div className="p-6 rounded-2xl glass-panel border border-white/5 mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
           ...
           <div className="flex items-center gap-3">
            <label htmlFor="college-select" className="text-xs font-semibold text-zinc-400 whitespace-nowrap">
              Filter by College:
            </label>
            <select
              id="college-select"
              ...
              className="rounded-xl border border-zinc-850 bg-zinc-950 px-3.5 py-2.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-indigo-500 cursor-pointer"
            >
              {colleges.map((c) => (
         ...
         <div className="rounded-2xl glass-panel border border-white/5 overflow-hidden">
          {isLoading ? (
            <div className="py-24 text-center text-sm text-zinc-500 flex flex-col items-center gap-3">
              ...
              <span>Fetching placements rankings...</span>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-zinc-850 bg-zinc-900/30 text-xs font-bold text-zinc-400 uppercase tracking-wider">
                ...
                <tbody className="divide-y divide-zinc-850">
                  {fileteredLeaderboard.length > 0 ? (
                    fileteredLeaderboard.map((student) => {
